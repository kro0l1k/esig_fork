...
def var_strat(level: int, T: float, M: int = None) -> float:
    """
...
def var_control(level: int, T: float, M: int = None) -> float:
    """
    Variance of control. 
    """
    if M is None:
...
def cov_strat_control(level: int, T: float, M: int = None) -> float:
    """
...
def corr_strat_control(level: int, T: float, M: int = None) -> float:
    return cov_strat_control(level, T, M) / np.sqrt(var_control(level, T, M) * var_strat(level, T, M))


def loose_lower_bound_corr(level: int) -> float:
