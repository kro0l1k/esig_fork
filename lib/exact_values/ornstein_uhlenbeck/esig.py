...
def expected_signature_OU(j: Union[int, Tuple[int, ...]], T: float, A: np.ndarray, Sigma: np.ndarray = None, method: str = "numeric") -> Union[np.ndarray, float]:
    """
    ...
    if isinstance(j, int):
        if j == 0:
            ...
            if method == "symbolic":
                ...
                Id = np.eye(d)
                M = np.zeros([2*d, 2*d])
                # apply Van Loan's trick
                M[:d, :d] = - A
                M[:d, d:] = Sigma
                M[d:, d:] = A.T
                Inf = - np.log(1e-10) / np.linalg.norm(A)
                expM = scipy.linalg.expm(M * Inf)
                Sigma_X = expM[d:, d:].T.dot(expM[:d, d:])
                return Sigma_X @ A.T * T + Sigma_X @ (Id - scipy.linalg.expm(A.T * T)) + 0.5 * Sigma * T    
        elif j == 3:
            return np.zeros((d,)*3)
        else:
...
def expected_signature_MCAR(j: Union[int, Tuple[int, ...]], T: float, AA: Union[Tuple[Union[float, np.ndarray], ...], List[Union[float, np.ndarray]]], Sigma: Union[np.ndarray, None] = None, method: str = "numeric") -> Union[np.ndarray, float]:
    """
    ...
    if isinstance(j, int):
        if j == 0:
            ...
            return expected_signature_OU(j, T, A_OU, Sigma_OU, method=method)[tuple(slice(d) for _ in range(j))]
    elif isinstance(j, tuple):
        if j == tuple():
            return 1.
