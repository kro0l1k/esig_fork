...
def expected_signature_linear_approx_BM(j: Union[int, Tuple[int, ...]], M: int, T: float, d: int):
    if isinstance(j, int):
        if j == 0:
            return 1
        rho = np.zeros((d,)*j)
...
def expected_signature_BM(j: Union[int, Tuple[int, ...]], T: float, d: int):
    if isinstance(j, int):
        if j == 0:
            return 1
        rho = np.zeros((d,)*j)
...
def var_estimator(I: tuple[int], M: int, T: float, d: int):
    if M is None:
        expected_signature = expected_signature_BM
    elif isinstance(M, int):
        expected_signature = partial(expected_signature_linear_approx_BM, M=M)
    variance = 0
    higher_orer_terms = expected_signature(j = 2*len(I), T=T, d=d)
    for J in shuffle(I, I):
        variance += higher_orer_terms[J]
